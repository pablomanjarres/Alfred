...
expectThrows({ _ = try AlfredCLIConfig(nodePath: "/bin/node", cliPath: "/tmp/cli.js", stateHome: "relative") }, "relative state directory should be rejected")
let handedOff = try StandbySnapshot.decode(#"{"loaded":true,"running":false,"state":{"status":"handed-off","detail":"End the call, then choose Start listening.","updatedAt":"2026-09-13T04:50:00.000Z"},"detail":"Paused"}"#).menuState(now: now)
expect(handedOff.kind == .handedOff, "completed handoff stays visible after standby exits")
expect(!handedOff.micIndicator, "handoff never shows Alfred microphone as active")
expect(handedOff.label == "Voice chat in Codex", "handoff label identifies the order surface")
